...
class PendulumController(rcp.doublependulum.DoublePendulumController):
    def __init__(self):
        ...
        self.keyframes = np.zeros((5,2))
        self.keyframes[0,0] = np.pi
        self.keyframes[0,1] = np.pi
        self.keyframes[1,0] = 0
        self.keyframes[1,1] = -1.5
        self.keyframes[2,0] = 1.35
        self.keyframes[2,1] = 2.2
        self.keyframes[3,0] = 1.35
        self.keyframes[3,1] = 2.2
        self.keyframes[4,0] = 0
        self.keyframes[4,1] = 0
...
def compute_control(self, t, dt, state, tau):
    ...
    tau[0] = ((self.kp[0] * (1 - (t/60))) * qerr[0]) + (self.kd[0] * qerr[2])
    tau[1] = ((self.kp[1] * (1 - (t/60))) * qerr[1]) + (self.kd[1] * qerr[3])
